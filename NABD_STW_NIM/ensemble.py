...
# model_b3 = models.resnet152()
# model_b3.load_state_dict(torch.load('model/resnet152-394f9c45.pth'))
# model_b4 = models.resnet101()
# model_b4.load_state_dict(torch.load('model/resnet101-63fe2227.pth'))
...
# def input_color(image, div_prob):
#     color = transforms.ColorJitter(brightness=0.5, contrast=0.2, saturation=0.5, hue=0.1)
#     if random.random() > div_prob:
#         return image
#     padded=color(image)
#     return padded
...
def nabd_stw_nim_attack(image, epsilon,t,target,model,nc,model_b1):
    decay_factor = 1
    ...
    for i in range(t):  # 共迭代 t 次
        ...
        fz=flip(nes_advdata)
        outputs_fz = (model(fz)+model_b1(fz))/3
        loss = F.cross_entropy(outputs_fz, target)
        loss.backward(retain_graph=True)
        noise_fz = nes_advdata.grad  # .detach()
        py=input_diversity(nes_advdata, low=266, high=299, div_prob=0.5)
        outputs_py = (model(py)+model_b1(py))/3
        loss = F.cross_entropy(outputs_py, target)
        loss.backward(retain_graph=True)
        noise_py = nes_advdata.grad  # .detach()
        cd=input_s(nes_advdata, div_prob=0.5)
        outputs_cd = (model(cd)+model_b1(cd))/3
        loss = F.cross_entropy(outputs_cd, target)
        loss.backward(retain_graph=True)
        noise_cd = nes_advdata.grad  # .detach()
        xz=input_x(nes_advdata,0.5)
        outputs_xz = (model(xz)+model_b1(xz))/3
...
def test( model, device, test_loader, epsilon ,model_b1,model_b2):
    ...
    for data, target in test_loader:
        ...
        # 唤醒攻击
        perturbed_data = nabd_stw_nim_attack(data,  epsilon, 5, target ,model,nc,model_b1)
        # 重新分类受扰乱的图像
...
for eps in epsilons:
    eps=eps/255
    acc,acc1,acc2,acc3, ex = test(model, device, img_iter, eps,model_b1,model_b2)
